...
18. **Dashboard aggregate caching: not built, and the reason narrowed.** `/dashboard/stats` measures 3 statements and passes its ceiling; it was never the bottleneck, and that is decisive on its own.

    **Correcting the reasoning given alongside it.** The Phase 5 commit argued that an in-process cache would also be wrong because Render may run several instances, each with its own cache, so two tabs could disagree. **That does not apply here:** Render's Hobby tier is single-instance with no horizontal scaling, **and the start command now declares `--workers 1`**, so there is exactly one process and therefore exactly one cache. Both halves are required — one instance running four uvicorn workers would give four caches — and until 2026-07-31 the worker count was uvicorn's default rather than a declaration, so the conclusion rested on something undeclared and untested. Pinned in `render.yaml` and `backend/start.sh`, with `tests/test_process_model.py` failing the suite if either raises it. The same fact makes the in-process rate limiter correct on this deployment — see SECURITY_REVIEW.md residual risk #5, now narrowed for the same reason. Cross-instance inconsistency is a real objection to in-process state in general and simply not an objection *on this topology*; using it here was reaching for a second argument when the first was already sufficient.

    **And if caching is wanted, the candidate has changed.** Item 17 measures `/attack/matrix` and `/attack/heatmap` as the expensive endpoints now — transfer plus parsing, linear in corpus size, uncached, per request. Their data changes only when ingestion adds technique mappings, so a short TTL is defensible there in a way it never was for a 3-statement aggregate. Revisit after the item 17 measurement is taken against the production corpus, not before.

17. **`/attack/*` is fixed for statement count and now bounded by transfer and CPU instead — measured, and it degrades linearly.** Phase 5 took both endpoints from 41 statements to 2 by fetching every tagged IOC's `mitre_techniques` in one scan and counting in Python. That was the right call (a `LEFT JOIN ... JSON_CONTAINS` cannot use an index and becomes techniques × IOCs evaluated per pair on a *shared* host), but statement count was a proxy metric and it has done its job. The cost that remains is wire volume and parsing, uncached, per request:

    | Tagged IOCs | JSON on the wire | Parse + count, local | ≈ at Render's 0.1 CPU |
    |---|---|---|---|
    | 1,000 | 17 KB | 5 ms | ~50 ms |
    ...
    **The right long-term fix is schema, so it is blocked with the migration chain**: a normalised `ioc_techniques(ioc_id, technique_id)` join table, or an indexed generated column, turns this into a properly indexed `GROUP BY` that returns hundreds of rows instead of tens of thousands. Recorded in [the migration-chain design](docs/superpowers/specs/2026-07-30-migration-chain-repair-design.md) §6 rather than left to be rediscovered.
...
21. **DEFERRED PRODUCTION CLEANUP — dead enrichment rows, needing owner sign-off.** Two populations of rows that are permanently useless but that only a production write can remove. Grouped here because they share a decision, not a cause: both are deletes against a live table, and neither is urgent.

    * **~5,947 Shodan rows.** Every one is an error payload — the connector's library is commented out of `requirements.txt` and is not transitive, so `enrich()` returned `{"error": ...}` before any network call. As of 2026-08-17 the source is gated out of the registry (see CLAUDE.md), so these rows are now **orphaned**: no enricher will ever refresh or replace them, and Section E's selection excludes error payloads from refresh. They occupy 12.4% of the `enrichments` table and answer nothing.
    * **GeoIP `error_city` rows, count unknown.** Same shape — written whenever the MaxMind database was absent. The count is an owner query (`SELECT COUNT(*) FROM enrichments WHERE source='geoip' AND JSON_EXTRACT(data,'$.error_city') IS NOT NULL`) and is no longer load-bearing for Section E, which excludes them either way.

    **Not done here, deliberately.** A delete against a live `enrichments` table is a production write, in the same class as the rescore and the history purge. It is also not urgent: the rows are inert rather than harmful now that the refresh selection skips them, so the cost of leaving them is storage and a misleading row count, not wrong behaviour.

    **One thing to check before deleting**, so it is not discovered afterwards: `_legacy_assessed` reads stored payloads for rows written before the `assessed` contract, and CLAUDE.md records that fallback as *permanent* because cached rows are never refreshed. Confirm no scoring path treats the absence of a row differently from the presence of an error row before removing them — the zero-evidence guard returns the 20.0 neutral for "no enrichment", and an error row is already excluded from `assessed`, so the two should agree. Should, not do: verify it.
...
15. **LIVE, found 2026-07-31 — one OTX pulse scores an indicator *lower* than no OTX pulse at all.** The reputation enricher maps OTX to `min(pulse_count * 10, 100)`, so a single pulse yields `aggregate_score = 10` and two yield 20 — both **below `NEUTRAL_REPUTATION = 30`**, the value used when there is no reputation evidence whatsoever. Measured identically across `domain`, `ip`, `url` and `hash`:
    ...
    **Decided in advance, 2026-07-31, so the section does not stall on a thin query:**
    ...
    - **Fallback `k = 14`**, from a stated judgement rather than from the data: **five pulses should read as certainly malicious.** Five independent write-ups is a well-documented indicator. `NEUTRAL + min(pulses * 14, 70)` reaches the 100 ceiling at exactly five pulses and gives 1 → 44, 2 → 58, 3 → 72, 4 → 86 — so a single pulse sits meaningfully above the 30.0 neutral without being conclusive, and the ladder stays monotonic across the range that actually occurs.

      **Corrected from `k = 20`, which contradicted its own rationale.** At 20 the ceiling arrives at *four* pulses (`30 + min(80, 70) = 100`), and the table published alongside it skipped 4 — which is exactly where the mismatch would have shown. Either value is defensible, but the constant has to follow the stated reason rather than drift from it, since the reason is the only thing a later reader can argue with. 14 is what "five pulses" means.

      Anchored the same way `MIN_ASSESSED_POINTS = 3` was: a judgement stated as a judgement, with the arithmetic shown so it can be checked.
