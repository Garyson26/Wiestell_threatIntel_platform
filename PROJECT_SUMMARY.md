...
11. **Hash IOCs are now the least-served type in the platform.** Three changes composed, none wrong individually:
    ...
    **Not implemented: weights need owner sign-off.** Recorded as the concrete recommendation for Spec 5 §6, with `tests/test_scoring_and_export.py::TestMalwareBazaarSignals::test_a_confirmed_family_still_does_not_reach_high` pinning the present behaviour so the change is deliberate. Note also that a hash profile would partly relieve the `reputation` problem by weight alone, but not fix it — the underlying gap is that no configured provider can return a corroborated verdict for a hash.
