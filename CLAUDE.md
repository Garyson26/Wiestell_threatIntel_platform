...
- **Six subsystems assume one process per instance, so the uvicorn worker count is part of the security model, not a throughput dial.** `--workers 1` is declared in `render.yaml` and `backend/start.sh`, refused at startup by `main.py::_assert_single_worker` (overridable with `ALLOW_MULTIPLE_WORKERS=true`), and pinned by `tests/test_process_model.py`. At N workers: the in-memory rate limiter becomes N independent budgets, so the effective limit is N × `AUTH_RATE_LIMIT_MAX` — this is what makes the residual-risk #5 narrowing valid; the database pool is per process, so 2 + 3 becomes 5N connections against a **shared** MySQL account allowance; `enrichment_engine`'s module-level semaphore becomes 5N third-party calls in flight; and any in-process cache becomes N inconsistent caches. **And `REDIS_URL` is deliberately unset** (the Render blueprint provisions no Redis), which is only correct because the limiter's fallback is process-local — at N workers a shared store stops being optional. **And `utils/email_service` tracks Resend quota exhaustion in module state**, so a cap hit by one worker is invisible to the others and to `/cron-status`. Each degrades silently. The check is at runtime as well as in a test because Render's dashboard can override the start command without touching a file. Raising the count means revisiting all six together — provision Redis for the limiter, any cache and item 5, a smaller `pool_size`, and a shared bound for enrichment. The start commands also pass `--no-proxy-headers`: uvicorn's own proxy middleware is a second algorithm on `X-Forwarded-For` and would fight `deps.py::_client_ip` (finding R-05).
- **A test that asserts on TIME needs a measurable interval, or it asserts nothing.** A
  narrower cousin of the extraction rule below, and it has now bitten twice. Section B's
  first read-chunk test measured a *first* sync, where two of the three reads short-circuit
  on an empty id list — coupled 77 statements against decoupled 63, both under its ceiling,
  so it passed with the feature reverted. Section D's cursor test asserted the cursor was
  the fetch's START time using a fake HTTP client that returned instantly, so start and
  finish were the same microsecond and the assertion held either way. In both cases the
  mutation that should have failed it failed *zero* tests. So: give the fake real latency
  (`await asyncio.sleep(0.05)` per page), or put the system in the state where the two
  branches actually diverge, and assert against **measured elapsed time** rather than a
  fixed epsilon — an epsilon large enough to be safe is usually large enough to be vacuous.
...
- Gate registration on the credential so an unconfigured source never appears applicable and never stores an `{"error": "no api key"}` row against every IOC. **Shodan was the documented exception to this rule until 2026-08-17, when the exception was RETIRED** — recorded here rather than deleted, because the reasoning is the useful part.

  The exception said Shodan "stays registered and returns an explanatory payload, preserving pre-existing dashboard behaviour". That holds only while the *library is present and the key is the one missing piece*: an explanatory payload is useful when the reader can act on it by adding a key. But `shodan` is commented out of `requirements.txt` ("not compatible with Vercel serverless (no binary wheels)") and is not pulled in transitively, so **in the deployed environment the payload is `{"error": "Shodan library not installed"}` even when the key IS set** — pointing an operator at the credential when the package is the problem. The pre-existing dashboard behaviour the exception protected had already stopped existing; what it was preserving was a misleading message.

  Measured cost before retirement: **5,947 rows, 12.4% of the enrichment table**, none of which ever carried data, each with a 6-hour `expires_at`. Registration is now gated on `importlib.util.find_spec("shodan")`; the key check survives inside `enrich()` as a second barrier, because importable-but-unconfigured is a real state locally and there the payload *is* actionable.

  **The gate is invisible in a dev environment** — `shodan` usually imports fine there, so `build_registry()` behaves exactly as before and nothing in the suite would notice the production difference. `tests/test_enrichment_selection.py` therefore forces the unavailable branch, and resets the cached registry around it (`reset_registry`) so a gated-out registry cannot leak into later tests and weaken them silently.
